...
- **每 Sandbox 独立私网**：容器类 runtime 使用独立 netns、veth、私有地址和 NAT 出口。内部端口不做全局预留，`exposedPorts` 已废弃。
...
Fast Sandbox 不再定义新的 Exec/File wire protocol。`fastctl logs` 仍用于 runtime diagnostics；用户进程日志和文件属于注入组件协议。
...
新客户端必须发送稳定的 Create `request_id`。旧 `consistency_mode`、`exposed_ports` 和 endpoint 数组只保留 v1alpha1 兼容读取，不再影响调度、持久化顺序或路由。

旧 Pool manifest 可以本地检查和转换，不会连接或修改集群：
