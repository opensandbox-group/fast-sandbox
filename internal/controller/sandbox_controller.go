...
type SandboxReconciler struct {
	...
	// Transitional construction fields. New wiring passes Orchestrator.
	Registry      fastletpool.FastletRegistry
	FastletClient sandboxorchestrator.FastletClient
...
func (r *SandboxReconciler) Reconcile(ctx context.Context, request ctrl.Request) (ctrl.Result, error) {
	...
	if sandbox.Status.Phase == string(apiv1alpha1.PhaseExpired) {
	...
	if sandbox.Status.Phase == string(apiv1alpha1.PhaseLost) && sandbox.Spec.FailurePolicy != apiv1alpha1.FailurePolicyAutoRecreate {
		return ctrl.Result{}, nil
	}
...
func (r *SandboxReconciler) reconcilePodLost(ctx context.Context, orchestrator *sandboxorchestrator.Orchestrator, sandbox *apiv1alpha1.Sandbox) (ctrl.Result, error) {
	...
	err := r.patchStatus(ctx, sandbox, func(status *apiv1alpha1.SandboxStatus) {
		status.Phase = string(apiv1alpha1.PhaseLost)
		status.RuntimeState = apiv1alpha1.ObservedStateUnavailable
		status.DataPlaneState = apiv1alpha1.ObservedStateUnavailable
		setSandboxCondition(status, sandboxorchestrator.ConditionRuntimeReady, metav1.ConditionFalse, "FastletPodLost", "The assigned Fastlet Pod no longer exists")
	})
	return ctrl.Result{}, err
}
...
func (r *SandboxReconciler) reconcileDeletion(ctx context.Context, orchestrator *sandboxorchestrator.Orchestrator, sandbox *apiv1alpha1.Sandbox) (ctrl.Result, error) {
	...
	if !done {
		_ = r.patchStatus(ctx, sandbox, func(status *apiv1alpha1.SandboxStatus) {
			status.Phase = string(apiv1alpha1.PhaseTerminating)
...
func (r *SandboxReconciler) reconcileExpiration(ctx context.Context, orchestrator *sandboxorchestrator.Orchestrator, sandbox *apiv1alpha1.Sandbox) (ctrl.Result, error) {
	...
	err = r.patchStatus(ctx, sandbox, func(status *apiv1alpha1.SandboxStatus) {
		status.Phase = string(apiv1alpha1.PhaseExpired)
		status.RuntimeState = apiv1alpha1.ObservedStateStopped
		status.DataPlaneState = apiv1alpha1.ObservedStateStopped
		setSandboxCondition(status, sandboxorchestrator.ConditionRuntimeReady, metav1.ConditionFalse, "Expired", "Sandbox desired lifetime expired")
	})
	return ctrl.Result{}, err
}
...
func (r *SandboxReconciler) orchestrator() (*sandboxorchestrator.Orchestrator, error) {
	if r.Orchestrator != nil {
		return r.Orchestrator, nil
	}
	registry, ok := r.Registry.(sandboxorchestrator.Registry)
	if !ok || r.FastletClient == nil {
		return nil, errors.New("Sandbox orchestrator is not configured")
	}
	return &sandboxorchestrator.Orchestrator{
		Client: r.Client, Registry: registry, FastletClient: r.FastletClient, Catalog: r.Catalog,
	}, nil
}
