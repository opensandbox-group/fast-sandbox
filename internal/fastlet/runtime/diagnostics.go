...
import "fast-sandbox/internal/api"
...
func (m *SandboxManager) recordDiagnosticLocked(sandboxID, level, source, phase, message string) {
	...
	events = append(events, api.SandboxDiagnosticEvent{
		Timestamp: m.clock.Now(), Level: level, Source: source, Phase: phase, Message: message,
	})
	if len(events) > maxDiagnosticEvents {
		events = append([]api.SandboxDiagnosticEvent(nil), events[len(events)-maxDiagnosticEvents:]...)
	}
	m.diagnostics[sandboxID] = events
}

func (m *SandboxManager) SandboxDiagnostics(req *api.SandboxDiagnosticsRequest) (*api.SandboxDiagnosticsResponse, error) {
	if failure := m.validateIdentityTarget(diagnosticsIdentity(req)); failure != nil {
		return &api.SandboxDiagnosticsResponse{Error: failure}, failure
	}
	m.mu.RLock()
	defer m.mu.RUnlock()
	...
	if metadata != nil {
		...
		if !deleted || tombstone.InstanceGeneration != identity.InstanceGeneration ||
			...
			failure := fastletError(api.ErrorNotFound, "Sandbox diagnostics are not retained by this Fastlet", false)
			return &api.SandboxDiagnosticsResponse{Error: failure}, failure
		}
	}
	...
	response := &api.SandboxDiagnosticsResponse{Events: append([]api.SandboxDiagnosticEvent(nil), events...)}
	if metadata != nil {
...
func diagnosticsIdentity(req *api.SandboxDiagnosticsRequest) *api.SandboxIdentity {
	if req == nil {
		return nil
	}
	return &req.Identity
