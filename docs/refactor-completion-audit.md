...
| 2 | Sandbox UID 是 runtime 全局逻辑身份 | assignment、Ensure、runtime label 均传递 CRD UID | `TestEnsureAssignmentAndRuntimeUseDurableUIDAndFences`；controlplane e2e 校验 assignment 与 CRD UID fencing | 已覆盖；最终快照定向 e2e 已通过 |
...
| 11 | 相同 request ID + spec 只对应一个 Sandbox | request ID label/hash、API Server create conflict/CAS | `TestConcurrentSameRequestConvergesToOneCRDAndUID`、`TestCreateIsIdempotent`、controlplane e2e | 已覆盖；最终快照 controlplane e2e 已通过 |
...
| 18 | 新 Pod 不接管旧 Pod runtime | assignment 同时绑定 Pod name + UID；PodLost 走 generation rebuild；containerd 发现旧 identity 时先回收再创建 | `TestReplacementPodWithSameNameCannotClaimOldAssignment`、`TestSameRuntimeIdentityIncludesPodAndGenerationFences`、`TestCleanupDecisionTreatsSameNameReplacementAsDifferentPod`、严格 AutoRecreate e2e | 已覆盖；最终快照 fault-recovery e2e 已通过 |
...
- 生产 OpenSandbox execd 是重点 Infra 接入 case，artifact、签名和版本绑定后续按接入指南完成；E2B envd 不作为重点，可删除或只保留 fail-closed 示例；
