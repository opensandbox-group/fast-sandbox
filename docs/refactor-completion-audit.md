...
| 16 | Create 成功要求 required Infra 和 local route Ready | Fastlet 原子 Create 依次完成 runtime、Infra 和 route publication，全部 Ready 才返回 Created；Controller 异步投影 `DataPlaneReady` | `TestRoutePublicationGatesCreateAndRetriesWithoutRecreatingRuntime`、`TestResolveEndpointRequiresDataPlaneReady`、Infra/官方 OpenSandbox SDK e2e | 已覆盖；最终快照 SDK e2e 已通过 |
