...
- RPC Create 使用 `request_id == metadata.name` 幂等；FastPath happy path 是 API Server Create CRD + Fastlet 原子 Create 两次 IO。CRD 创建前可判定的失败不留对象，IO1 后的失败保留声明式 intent 和 durable assignment；
...
- Core 不定义 Exec/File 协议，通过 Infra Component 注入、服务发现、鉴权和透明代理提供能力；`fastctl opensandbox exec/cp/files` 及 Go Adapter 直接使用官方 OpenSandbox Go Execd SDK，E2B envd 专属实现已删除；
- `fastctl diagnostics sandbox` 提供平台生命周期诊断事件，明确不冒充 Sandbox 内进程 stdout；Fastlet 不可达时返回降级诊断快照而不是掩盖控制面事实；
...
- Containerd 删除采用 ensure-absent 状态机，自行退出的 workload、已缺失 task/container/snapshot 和重复删除均收敛成功，Controller finalizer 不再停留在 Draining；SIGKILL 后等待退出事件再删除 task。旧 Fastlet runtime 接管失败时使用独立 `create-cleanup-failed` 状态续做 cleanup，普通 `delete-failed` 不允许被 Create 复活；
...
| 旧 Fastlet runtime 接管 | PASS | AutoRecreate 删除 Fastlet Pod 后，新 Fastlet ensure-absent 旧 containerd task/runtime，再以新 Pod UID、generation 和 runtimeInstanceID 恢复 Ready；创建补偿和用户删除使用不同失败状态 |
...
```bash
# fast:~/fast-sandbox
make test-unit GOFLAGS=
make test-race GOFLAGS=
go build -mod=mod ./...
make test-network-integration GOFLAGS=
# 全部 exit 0

E2E_TEST_TIMEOUT=30m make test-e2e-controlplane GOFLAGS=
E2E_TEST_TIMEOUT=15m make test-e2e-drain GOFLAGS=
E2E_TEST_TIMEOUT=15m make test-e2e-faultrecovery GOFLAGS=
E2E_TEST_TIMEOUT=10m make test-e2e-cliintegration GOFLAGS=
E2E_TEST_TIMEOUT=10m make test-e2e-advancedfeatures GOFLAGS=
E2E_TEST_TIMEOUT=20m make test-e2e-secureruntime GOFLAGS=
# controlplane 聚合命令执行到 scheduling 时暴露旧断言；修正后 scheduling
# 和 cleanupjanitor 整包分别重跑。10 个 suite 最终全部 exit 0。

# 本地纯 Python Gate
UV_CACHE_DIR=/private/tmp/fast-sandbox-uv-cache PYTHONPATH=sdk/python \
  uv run --offline --with grpcio==1.76.0 --with protobuf==6.33.5 \
  python -m unittest discover -s sdk/python/tests -v
# exit 0; Ran 3 tests; OK
```

Go protobuf、DeepCopy、CRD 在远端重新生成，生成前后逐文件 SHA-256 相同；Python protobuf 使用 `grpcio-tools==1.76.0` 离线重新生成，生成前后逐字节一致。
...
| 控制面三形态 | 完整 `test/e2e/suites/controlplane` | PASS，最终回归 `122.848s` |
| Linux 私网/NAT | `test-network-integration` 的编译、test image 和 privileged run 三步 | PASS，privileged test `0.18s` |
| Kubernetes 基础能力 | 完整 `test/e2e/suites/basicvalidation`：canonical CRD、私网/NAT、Proxy、Infra、官方 OpenSandbox SDK、warmImages | PASS，`461.958s` |
| Lifecycle | 同名重建、自退出 workload 删除、正常 graceful deletion | PASS，`162.622s`；自退出核心断言 `7.06s` |
| Scheduling | durable assignment 下的 capacity 与 CAS 扩容改派 | PASS，`91.941s`；扩容核心断言 `8.55s` |
| Drain | 完整 `test/e2e/suites/drain` | PASS，`161.781s` |
| PodLost/Reset | 完整 fault-recovery suite，包括 expiry、严格 AutoRecreate、reset、orphan | PASS，`461.819s`；修复后 AutoRecreate `73.81s` |
| Janitor | 完整 cleanupjanitor suite | PASS，`101.170s` |
| gVisor / Kata / runtime boundary | 完整 `test/e2e/suites/secureruntime` | PASS，`429.727s` |
...
| Python SDK | Codex 工作区 Python + uv 离线临时依赖环境 | PASS，`3 tests` |
| fastctl CLI | run/get/update/reset 与 config-file；OpenSandbox/diagnostics 由 unit + SDK E2E 覆盖 | PASS，CLI E2E `116.624s` |
| Advanced Infra wiring | platform-owned InfraProfile 注入 Fastlet Pod | PASS，`32.475s` |
...
- 最终 `make test-network-integration` 完整执行 Fastlet image build、network test image build 和 privileged run，`TestLinuxNetNSDriverPrivileged` 在 `0.18s` 通过；
...
- fault-recovery Gate 复现出 SIGKILL 后尚未确认退出就删除 task、随后 `delete-failed` 不续做 cleanup 的问题。最终实现等待强杀退出，并区分可恢复的 `create-cleanup-failed` 与不可由 Create 复活的用户 `delete-failed`；完整 fault-recovery 复验通过；
- macOS 系统 Python 不作为 runtime 结论来源；Python SDK 在 uv 离线隔离环境中执行三项 lifecycle/request_id/route/telemetry 单元测试并通过。
...
- 生产 OpenSandbox execd artifact release/signature binding 按接入指南后续完成；execd 是重点方向。E2B envd 的内置 profile、专属 Adapter 和 SDK 入口已删除；
