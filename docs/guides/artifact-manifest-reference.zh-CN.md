...
| `lineage` | 谱系对象：这套制品**从哪来、构建期烘焙了什么**。初构写入后，快照/checkpoint 逐字继承（只有 `lineage.image` 被覆盖为每一代启动所用的引用，`imageDigest` 永远指向最初构建），是跨代制品的"血缘档案" |
| `lineage.image` | 来源镜像引用。模板构建写入模板的 `spec.image`；快照/checkpoint 覆盖为拍摄该 Sandbox 当时的 `image` 引用（若它是从模板名创建的，就是那个模板名） |
| `lineage.imageDigest` | 初次构建时源 OCI 镜像的 digest。任何一代都不覆盖它，因此无论快照传多少代，它始终锚定最初那次构建 |
| `lineage.execd` | 构建时注入 guest rootfs 的 OpenSandbox execd 镜像引用。注入发生在构建期，此字段是谱系记录（说明 rootfs 里带着哪个 execd），运行时不重新解析 |
| `lineage.kernel` | guest 内核：`name` 为构建镜像内置的内核文件名，`digest` 为该文件的 SHA-256。内核是构建期资产（不是节点运行时资产）；恢复时使用的内核由节点 runtime 环境提供，digest 用于核对 |
