...
| `lineage.kernel` | Guest kernel: `name` is the kernel file baked into the builder image, `digest` is its SHA-256. The kernel is a build-time asset (not a node runtime asset) and rides forward through snapshots; at restore the node runtime environment supplies the kernel and the digest serves as a cross-check |
...
| `lineage.envs` | The template envs published **verbatim** (no OCI `Config.Env` merge, no `valueFrom` support); written into the guest's `/etc/sandbox-init.env` at build time. Never put secrets here — anyone who can read the manifest can read these values; credentials belong in `publishSecretRef` |
