...
`e2b-envd` 不再作为当前重点支持方向，只保留为另一种 Infra Component 生命周期案例：不可变 binary 预装在 VM template，实例身份通过 `/init` 注入，初始化后再发布路由。

当前内置 `e2b-envd` 标记 `Configured=true`，但仓库没有证明实际 VM template 已预装匹配版本，这是过早的能力声明。后续有两个选择：

1. 没有明确消费者时删除内置 profile 和专属测试；
2. 仅作为示例保留时改为 `Configured=false`，要求部署绑定 template capability。

本轮不继续投资 envd，也不以 envd 阻塞 execd 接入。
