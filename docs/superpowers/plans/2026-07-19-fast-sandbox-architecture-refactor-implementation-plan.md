...
- BoxLite v0.9.7 只提供整数 vCPU 和 memory knobs，没有可验证的 guest PIDs limit；fractional CPU 也尚未有 per-Box host throttle。因此 `resource-limits-v1=false`，native Sidecar 仍为 `Ready=false`。内置 `boxlite` 的 capability reason 继续使用 `BoxLiteResourceEnforcementIncomplete`，Pool 保持 `RuntimeReady=False/RuntimeUnsupported`。当前代码的远端命令 `E2E_PROFILE=basic E2E_TEST_TIMEOUT=20m make test-e2e-runtime-boxlite` 退出状态 `0`，总耗时 `115.818s`，Pool condition 精确断言上述 reason；该结果只证明 fail closed，不是 BoxLite 已支持；
