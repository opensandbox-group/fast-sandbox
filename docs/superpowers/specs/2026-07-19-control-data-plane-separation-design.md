...
> 2026-07-23 更新：Create 成功条件以 [RuntimeReady 快速返回语义](./2026-07-23-runtime-ready-create-semantics.md) 为准；本文中 required Infra 继续 gate DataPlaneReady，但不再 gate Create RPC。
...
### 2.7 required Infra Component 进入 DataPlaneReady 条件

如果 InfraProfile 中的组件被声明为 `required`，DataPlaneReady 只有在以下条件满足后才能成立：
...
CreateSandbox 在 RuntimeReady 后返回。required 或可选组件失败都必须通过独立 DataPlane Condition 和 diagnostics 暴露；required 失败禁止发布可用路由。
...
Fastlet Ensure Sandbox
  ...
  -> start runtime
  -> CreateSandbox returns success (RuntimeReady)
...
默认情况下 Infra Component 与用户进程并行启动；只有 `startBeforeUser=true` 的组件才阻塞用户 entrypoint。无论用户进程是否并行启动，路由都只能在 runtime 和 required Infra Component ready 后发布，但 CreateSandbox 不等待该异步阶段。
...
CreateSandbox 在 RuntimeReady 后返回，因此用户观察到的 Create 延迟近似为：
...
DataPlaneReady 在 Create 返回后继续异步推进：
...
相对没有 Infra Component，Create 同步路径的额外延迟主要是：
...
这样用户 entrypoint 可以接近原始启动时刻，CreateSandbox 在 RuntimeReady 后返回，DataPlaneReady 异步收敛。仅当组件必须先修改 mount、CA、环境或安全策略时才串行启动。
...
13. required Infra Component 完成 instance init、readiness 和 local route publication 是 DataPlaneReady 条件；CreateSandbox 在 RuntimeReady 后返回；
...
16. Create 热路径不等待 Infra readiness、route publication 或全局 Registry 收敛；
