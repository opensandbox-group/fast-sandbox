...
> 2026-07-23 更新：第 5 节同步成功条件已由 [CreateSandbox RuntimeReady 快速返回语义](./2026-07-23-runtime-ready-create-semantics.md) 修订。
...
Sandbox CRD 已创建，UID 已确定
  -> assignment annotation 已随 CRD Create/CAS 提交
...
- required Infra Components 的 Instance Init 和 readiness；
- Fastlet Proxy local route publication 和 DataPlaneReady；
- 所有 Sandbox Proxy 副本完成 Watch 同步；
- 所有 Local Registry 收敛；
- 非 required Infra Component Ready。

required Infra 和 local route 由 Fastlet 异步推进。Controller 独立投影 `RuntimeState=Ready` 与 `DataPlaneState=Creating/Unavailable/Ready`。
...
CreateSandbox 同步成功以 RuntimeReady 为准；性能分析必须继续保留 Accepted、RuntimeReady、UserProcessStart 和 DataPlaneReady 等独立指标。
...
16. CreateSandbox 成功意味着 RuntimeReady；required Infra 和 Fastlet local route Ready 只决定 DataPlaneReady；
...
Create 成功以 RuntimeReady 为准，同时分别观测 Accepted、RuntimeReady、UserProcessStart 和 DataPlaneReady 延迟
```
