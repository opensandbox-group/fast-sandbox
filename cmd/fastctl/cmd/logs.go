...
var logsCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if info.FastletPod == "" {
			klog.ErrorS(nil, "Sandbox not assigned to any fastlet", "name", name)
			log.Fatal("Sandbox is not assigned to any fastlet yet.")
		}
		if info.SandboxId == "" {
			klog.ErrorS(nil, "Sandbox ID is not available yet", "name", name)
			log.Fatal("Sandbox ID is not available yet.")
		}
		klog.V(4).InfoS("Sandbox fastlet pod", "name", name, "fastletPod", info.FastletPod)

		// todo add proxy for fastlet
		localPort, pfCmd, err := startPortForward(info.FastletPod, namespace)
		if err != nil {
			klog.ErrorS(err, "Failed to start port-forward", "fastletPod", info.FastletPod)
			log.Fatalf("Failed to start port-forward: %v", err)
		}
		klog.V(4).InfoS("Port-forward started", "localPort", localPort, "fastletPod", info.FastletPod)
		defer func() {
		...
		// Use the actual sandboxID (hash) instead of name for Fastlet API
		url := fmt.Sprintf("http://localhost:%d/api/v1/fastlet/logs?sandboxId=%s&follow=%t", localPort, info.SandboxId, follow)
		klog.InfoS("Fetching logs from fastlet", "sandboxID", info.SandboxId, "url", url)
		...
		if err != nil {
			klog.ErrorS(err, "Failed to connect to fastlet", "url", url)
			log.Fatalf("Failed to connect to fastlet: %v", err)
		}
		defer resp.Body.Close()

		if resp.StatusCode != http.StatusOK {
			body, _ := io.ReadAll(resp.Body)
			klog.ErrorS(nil, "Fastlet returned error", "statusCode", resp.StatusCode, "body", string(body))
			log.Fatalf("Fastlet returned error: %s", string(body))
		}
